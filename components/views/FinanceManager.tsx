...
export const FinanceManager: React.FC<FinanceManagerProps> = ({ transactions, setTransactions }) => {
  ...
  const handleAddTransaction = () => {
    ...
    const amountVal = Number(newTransaction.amount);
    if(isNaN(amountVal)) return;

    const amount = newTransaction.type === 'expense' ? -Math.abs(amountVal) : Math.abs(amountVal);
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-1 h-fit">
          <h3 className="font-bold mb-4 text-slate-700">Nova Movimentação</h3>
          <div className="space-y-4">
            ...
            <Button onClick={handleAddTransaction} className="w-full justify-center">Registrar</Button>
          </div>
        </Card>
              ...
              <tbody className="divide-y divide-slate-100">
                {transactions.map(t => (
                  <tr key={t.id}>
                    ...
                    <td className={`py-3 text-right pr-2 font-bold ${t.type === 'income' ? 'text-green-600' : 'text-red-600'}`}>
